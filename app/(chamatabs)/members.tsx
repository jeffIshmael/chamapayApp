...
const GroupDetails = () => {
    ...
    const chama = {name:"chama1"};
    const members = [{address:"0xtwxwe..", name:"user1"}, {address:"0xtwxwe..", name:"user2"}, {address:"0xtwxwe..", name:"user3"}];
          ...
          {members.map((member, index) => (
            <View key={index} style={{ flexDirection: "row", alignItems: "center", marginTop: 8, paddingVertical: 8, borderBottomWidth: index < members.length - 1 ? 1 : 0, borderBottomColor: "#e0e0e0" }}>
              ...
              <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 16, fontWeight: "500", color: "#4e4e4e" }}>{member.name || "Loading..."}</Text>
                    <Text style={{ fontSize: 14, color: "#9e9e9e" }}>
                      {member.address ? `${member.address}` : "Loading..."}
                    </Text>
        
        
              </View>
              {/* {member === admin && <Text style={{ fontSize: 14, color: "#757575", textAlign: "right" }}>Admin</Text>} */}
            </View>
          ))}
        </View>
